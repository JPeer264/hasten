...
Base.prototype.addPackage = function addPackage() {
    ...
    // } else {
        ...
        fs.writeFile(path.join(dir, 'package.json'), result, 'utf8', function(err) {
            ...
            hasten.base.executeInShell(self.postinstall);
        });
 ...
 * execute in shell
 *
 * @alias module:base.executeInShell
 *
 * @param {Array} cmds - To execute commandos
 */
Base.prototype.executeInShell = function executeInShell(cmds) {
    // shell execution
    if (cmds === undefined && cmds.length <= 0) {
        return;
    }

    console.log('\nRun ' + (cmds.join(' & ')).bold.red + ' in terminal');

    // solution http://stackoverflow.com/questions/24483054/the-console-output-is-a-mess-while-executing-npm-install-in-child-process-exec
    var execthis = exec(cmds.join(' & '));

    execthis.stdout.pipe(process.stdout);
    execthis.stderr.pipe(process.stderr);
    return;
}
