...
function showInstalledFrameworks() {
    var localFrameworks = fs.readdirSync(hasten.paths.framework);
    var localModuleList = [];
    var msg = 'There is 1 Framework installed';

    if (localFrameworks.length === 0) {
        console.log('No installed frameworks.');
        return;
    }

    if (localFrameworks.length > 1) {
        msg = 'There are ' + localFrameworks.length + ' Frameworks installed:';
    }
    ...
    for (var dirName of localFrameworks) {
...
function listFrameworks(isInstall) {
    ...
    var pathToModules = path.join(process.cwd(), 'node_modules');
    var globalModules = fs.readdirSync(pathToGlobalModules);
    var localModules = fs.readdirSync(pathToModules);
    var localFrameworks = fs.readdirSync(hasten.paths.framework);
    var result = [];
    var localModuleList = [];
    ...
    // check all local frameworks, wheter installed or not
    // searchLocalModules:
    // for (var dirName of globalModules) {
    //     var frameworkName = dirName.substr(7);
    ...
    if (isInstall && result.length === 0) {
        ...
        return;
    } else if (!isInstall && localFrameworks.length === 0) {
